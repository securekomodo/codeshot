...
# Every frame is 864x432: height = card + pad + max(pad, 84), where 84 is the
# caption band. Kali frames have taller bars, terminal line spacing and
# two-line prompts, so their padding, width and font size are chosen to hit
# the same frame height; so is the milestone's.
...
02|Terminal Window|terminal|kali|slate|dejavu|--chrome kali --cursor --padding 36 --width 792
03|API Response|api|oneLight|paper|space|--no-badge --title reading.json
04|Error & Log|error-log|vsDark|ink|cascadia|--chrome kali --padding 87 --width 690 --title "kali@kali: ~"
